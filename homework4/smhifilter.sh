...
# Select only measurements taken on weekends (Saturday and Sunday)
FILTER_FILTERFILENAME_ONLYWEEKENDS="onlyweekends_$FILTER_FILTEREDFILENAME"
log "Filtering on only measurements taken on weekends, writing to $FILTER_FILTERFILENAME_ONLYWEEKENDS"

# Use awk to read and filter the data based on the day of the week
awk -F, '
{
    # Construct a command to run the "date" utility with the date from the CSV file (first field, $1)
    # -d "$1" tells date to convert the date in the first field into the day of the week
    # +%u outputs the day of the week as a number (1=Monday, 7=Sunday)
    cmd = "date -d " $1 " +%u"
    
    # Run the command stored in cmd, and capture the output (the day of the week as a number) into the variable "day"
    cmd | getline day
    
    # Close the command to free resources after each line is processed
    close(cmd)
    
    # Check if the day is 6 (Saturday) or 7 (Sunday)
    if (day == 6 || day == 7)
        # If it is, print the whole line ($0 represents the entire line)
        print $0
}' $CLEANER_BAREDATAFILENAME > $FILTER_FILTERFILENAME_ONLYWEEKENDS
